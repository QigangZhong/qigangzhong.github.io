...
其中服务暴露主要是通过ServiceBean来完成的，它实现了InitializingBean，所以afterPropertiesSet方法会被首先执行。同时它还实现了ApplicationContextAware，所以setApplicationContext方法会被调用，它还实现了ApplicationListener，onApplicationEvent方法也会被调用。
